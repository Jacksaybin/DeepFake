...
# Cache to avoid repeated metadata calls per frame
_CACHED_INPUT_NAME: str | None = None
_CACHED_ALIGN_SIZE: int | None = None
        ...
        if FACE_ENHANCER is None:
            ...
            try:
                from modules.processors.frame._onnx_enhancer import (
                    build_provider_config, warmup_session,
                )
                ...
                # Cache metadata once to avoid per-frame overhead
                global _CACHED_INPUT_NAME, _CACHED_ALIGN_SIZE
                input_info = FACE_ENHANCER.get_inputs()[0]
                output_info = FACE_ENHANCER.get_outputs()[0]
                _CACHED_INPUT_NAME = input_info.name
                try:
                    _CACHED_ALIGN_SIZE = int(input_info.shape[2])
                    if _CACHED_ALIGN_SIZE <= 0:
                        _CACHED_ALIGN_SIZE = 512
                ...
                print(f"{NAME}: CPU threads intra={cpu_count} inter={max(1, cpu_count // 2)}")

                # Warmup: trigger JIT so first real inference is instant
                warmup_session(FACE_ENHANCER)
...
def enhance_face(temp_frame: Frame) -> Frame:
    ...
    # Use cached metadata (populated once at session load)
    global _CACHED_INPUT_NAME, _CACHED_ALIGN_SIZE
    input_name = _CACHED_INPUT_NAME or session.get_inputs()[0].name
    align_size = _CACHED_ALIGN_SIZE or 512
